...
namespace UniqueRegionNamesPatcher.Utility
{
    ...
    public class RegionMap
    {
        ...
        public RegionMap(Stream stream, FormKey worldspaceFormKey, ref IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
        {
            ...
            var streams = SplitStream(stream);
            ParseRegions(streams[FileHeader.Regions], ref state);
        ...
        /// <summary>
        /// Parses the <see cref="FileHeader.Regions"/> header, and creates new <see cref="Region"/> records to be used during the main patcher process.
        /// </summary>
        ...
        private void ParseRegions((Stream, int) streamSection, ref IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
        {
        ...
        {
            Dictionary<FormKey, ulong> regionSizes = new();
            ulong totalSize = 0;

            foreach (var (_, regions) in Map)
            {
                foreach (var region in regions)
                {
                    if (!regionSizes.ContainsKey(region.FormKey))
                        regionSizes[region.FormKey] = 1;
                    else ++regionSizes[region.FormKey];
                }
                // increment total by one for each mapped coordinate
                ++totalSize;
            }

            KeyValuePair<FormKey, ulong>? largest_region = null;
            regionSizes.ForEach(r => largest_region = (largest_region == null || r.Value > largest_region.Value.Value ? r : largest_region));
            decimal largest_region_percentage = Convert.ToDecimal(largest_region?.Value) / Convert.ToDecimal(totalSize) * 100;
            ...
            foreach (var rw in Regions)
            {
                ...
                ulong size = regionSizes[region.FormKey];
                decimal percentage = Convert.ToDecimal(size) / Convert.ToDecimal(totalSize) * 100;

                decimal percentDiff = largest_region_percentage - percentage;

                regionCopy.Map!.Header!.Priority = Convert.ToByte(Math.Round(percentDiff.Scale(0, 1, 60, 55), 0));
